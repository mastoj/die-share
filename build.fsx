...
let serverConfig =
  { defaultConfig with
      ...
      bindings = [ HttpBinding.mkSimple HTTP "0.0.0.0" 8033] }
...
Target "spawn" (fun _ ->
    ...
    let buildFsx = projectRoot @@ "build.fsx"
    let fakeExe = projectRoot @@ "packages/FAKE/tools/FAKE.exe"
    let fakeArgs = "run --fsiargs " + buildFsx
